...
export const rank = createCommand({
  name: "rank",
  ...
  const rank =
    lvlDB.findIndex((x) => x.id === `${interaction.guild?.id}-${interaction.user.id}`) + 1;
  ...
  const url = `${env.IMG_BACKEND}/level?level=${level}&username=${username}&currentXP=${xp}&totalXP=${xpRequired}&rank=${rank}&mdAcc=${uid !== "unlinked"}&premium=${premium}&avatar=${avatar}&bg=${bg}&color=${color}`;
