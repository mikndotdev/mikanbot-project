...
export const dmEndpoint = new Elysia({ prefix: "/admin/dm" }).post("/", async ({ query, body }) => {
  const key = query.key;
  ...
  if (key !== env.API_SIGNING_KEY) return new Response("Invalid key", { status: 401 });
  ...
  if (response instanceof Error) return { status: 500, message: response.message };
